...
if(isset($_POST) && !empty($_POST)){
    ...
    if(isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['pseudo']) && !empty($_POST['pseudo']) && isset($_POST['pass']) && !empty($_POST['pass'])){
        ...
        // On écrit la requête
        $sql = 'INSERT INTO `users`(`email`, `password`, `pseudo`, `avatar`, `typing`, `online`) VALUES (:email, :password, :pseudo, :avatar, :typing, :online);';
        ...
        $query->bindValue(':avatar', 'img/yoshi.jpg', PDO::PARAM_STR);
        $query->bindValue(':typing', 2, PDO::PARAM_STR);
        $query->bindValue(':online', 2, PDO::PARAM_STR);
